...
*Location section (top — GPS is unreliable in the field, so the reporter can correct it):* a **"موقع الحادثة"** preview map centered on the current GPS fix at **city zoom** with a **location puck** so the reporter sees where they are, and the chosen point as an accent pin. A note underneath reads *"اضغط على الخريطة لتغيير موقع الحادثة"*. Tapping the preview opens a **bigger editor card** (bottom sheet): a taller interactive map where **tapping drops/moves the pin**, a **"موقعي الحالي"** button that re-acquires GPS and recenters, and — inside the same card — an optional **free-text place field** (*"أو اكتب وصف الموقع"*, e.g. *"قرب مدرسة الرشيد"*); a **"تم"** button applies and closes. The editor hint reflects *locating* / *GPS-failed* / *manually adjusted*. The submitted report carries the chosen coordinates and the optional `locationText`.
