...
A compact **rounded pill centered at the top of the map** (matching the safety status pill's style) showing the **current year's victim count** as a bold red number with the year label *"ضحايا 2026"*, loaded from the victims repository (`GET /victims/stats`, §10.3) with **loading + error** states. Tapping the card opens a lightweight **popover** floating over the map — a transparent overlay with a scrim backdrop, fade + scale animation, dismissed by an outside tap or the **×** (not a full-screen modal):
...
- **Custom period:** a single **calendar button** expands an in-app date-range calendar (`DateRangeCalendar`); picking a range totals victims across the years it spans and shows *"ضحايا في الفترة المحددة"* (auto-updates, `GET /victims?fromYear&toYear`).
...
*(Placement: the safety status pill is at the top-leading edge, the victims counter is centered at the top, and the inbox/settings toolbar is at the top-trailing edge — §5.7.)*
...
*Location section (top — GPS is unreliable in the field, so the reporter can correct it):* a **"موقع الحادثة"** preview map that **defaults to the user's current location** — the same fix the home map already holds in the map store, so it is correct immediately — shown **zoomed out to city level** with a blue **location puck** and the incident point as a slightly translucent accent pin that **starts exactly on the puck** — the pin seeds and tracks from Mapbox's own `locationManager` (the puck's coordinate source) until the reporter moves it, so red sits precisely on blue by default. A best-effort `getCurrentPosition` refreshes it and **silently keeps the best-known location on failure — there is no "failed to locate" state**. A note underneath reads *"اضغط على الخريطة لتغيير موقع الحادثة"*. Tapping the preview opens a **bigger editor card** (bottom sheet): a taller interactive map where **tapping drops/moves the pin**, a **"موقعي الحالي"** button that re-acquires GPS and recenters, and — inside the same card — an optional **free-text place field** (*"أو اكتب وصف الموقع"*, e.g. *"قرب مدرسة الرشيد"*); a **"تم"** button applies and closes. The submitted report carries the chosen coordinates and the optional `locationText`.
